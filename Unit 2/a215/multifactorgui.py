...
class MultiFactorAuth(tk.Tk):
    ...
    def authorize(self):
        ...
        if (student_username == self.username and student_password == self.password):
            ...
            self.frame_auth = tk.Frame(self, bg=user_answer)
            self.title("Authenticate")
            self.frame_auth.grid(row=0, column=0, sticky="news")

            self.lbl_auth = tk.Label(self.frame_auth, text=self.security_question + "?", bg=user_answer)
    ...
    def authenticate(self):
        ...
        if (factor_info == self.answer):
            ...
            lbl_auth = tk.Label(self.frame_restrict,
                                text="Congratulations!\nYou have authenticated, and that makes Jeff very happy.",
                                bg="sienna2")
    ...
    def reset_authorizaiton(self):
        ...
        if (student_username == self.username and student_password == self.password):
            ...
            self.btn_reset_auth = tk.Button(self.frame_reset, text='SET new Username and Password',
...
# prompts the user to configure a username, password, and favorite color
...
def valid_user():
    if (8 <= len(user) <= 24):
        ...
        valid_user == False
        print(
            "Please make sure your username meets length requirements. Usernames must be in between 8 and 24 characters in length.")
    ...
    if (8 <= len(pw) <= 24):
        if pw.isdigit() == True:
            if pw.isalpha() == True:
                ...
                valid_pass() == False
                print(
                    "Please make sure your password meets length requirements. Passwords must be in 8 and 24 characters in length and contain both letters and numbers.")
...
my_auth.set_authorization(user, pw)
